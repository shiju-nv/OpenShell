...
fn materialize_policy(directory: &Path, name: &str, port: u16) -> PathBuf {
    ...
    let policy = fixture(name)
        .replace("egress-a.invalid", TEST_HOST)
        .replace("egress-b.invalid", TEST_HOST)
...
// Decode only the platform-event fields needed by this fixture. The field tags
// follow WatchSandboxRequest, SandboxStreamEvent, and PlatformEvent; unrelated
// snapshots and logs are skipped by Prost.
#[derive(Clone, PartialEq, prost::Message)]
struct StartupWatchRequest {
    #[prost(string, tag = "1")]
    id: String,
    #[prost(bool, tag = "4")]
    follow_events: bool,
...
#[derive(Clone, PartialEq, prost::Message)]
struct StartupWatchResponse {
    #[prost(message, optional, tag = "3")]
    event: Option<StartupPlatformEvent>,
}

#[derive(Clone, PartialEq, prost::Message, serde::Serialize)]
struct StartupPlatformEvent {
    #[prost(int64, tag = "1")]
    timestamp_ms: i64,
    #[prost(string, tag = "2")]
    source: String,
...
async fn startup_watch_channel() -> Channel {
    // The Docker wrapper registers both its managed mTLS gateway and an
    // explicitly supplied plaintext endpoint in this same CLI configuration.
    let config_home = std::env::var_os("XDG_CONFIG_HOME").expect("Docker wrapper config home");
    let gateway_name = std::env::var("OPENSHELL_GATEWAY").expect("Docker wrapper gateway name");
    let gateway_dir = PathBuf::from(config_home)
        .join("openshell/gateways")
        .join(gateway_name);
    let metadata: Value = serde_json::from_slice(
        &std::fs::read(gateway_dir.join("metadata.json")).expect("Docker wrapper gateway metadata"),
    )
    .expect("gateway metadata JSON");
    let address = metadata["gateway_endpoint"]
        .as_str()
        .expect("Docker wrapper gateway endpoint");
    let mut endpoint = Endpoint::from_shared(address.to_string())
        .expect("valid Docker wrapper endpoint")
        .connect_timeout(Duration::from_secs(10));
    if address.starts_with("https://") {
        assert!(
            metadata["auth_mode"].is_null() || metadata["auth_mode"] == "mtls",
            "restart fixture requires the Docker wrapper's mTLS authentication"
        );
        let tls_dir = gateway_dir.join("mtls");
        let ca = std::fs::read(tls_dir.join("ca.crt")).expect("gateway CA certificate");
        let cert = std::fs::read(tls_dir.join("tls.crt")).expect("gateway client certificate");
        let key = std::fs::read(tls_dir.join("tls.key")).expect("gateway client private key");
        endpoint = endpoint
            .tls_config(
                ClientTlsConfig::new()
            ...
            .expect("configure authenticated startup watch");
    } else {
        assert!(
            address.starts_with("http://") && metadata["auth_mode"] == "plaintext",
            "restart fixture requires the Docker wrapper's explicit plaintext endpoint"
        );
    }
    endpoint.connect().await.expect("connect startup watch")
}

async fn wait_driver_started(sandbox_id: &str) -> StartupPlatformEvent {
    // Gateway activation can precede the driver's initial Docker health check.
    // Started is emitted after that check and control-process retention, so an
    // injected restart now exercises a running runtime instead of provisioning.
    let mut last_event = None;
    tokio::time::timeout(Duration::from_secs(120), async {
        let channel = startup_watch_channel().await;
        let mut client = tonic::client::Grpc::new(channel).max_decoding_message_size(1024 * 1024);
        client.ready().await.expect("startup watch channel ready");
        let request = tonic::Request::new(StartupWatchRequest {
            id: sandbox_id.to_string(),
            follow_events: true,
            event_tail: 50,
        });
        // stop_on_terminal is omitted (false): Ready must not end this watch.
        // The server subscribes before replaying its tail, covering either
        // ordering between driver completion and this fixture's subscription.
        let response = client
            .server_streaming(
                request,
                ...
                tonic_prost::ProstCodec::<StartupWatchRequest, StartupWatchResponse>::default(),
            )
            .await
            .expect("watch sandbox startup events");
        let mut stream = response.into_inner();
        while let Some(response) = stream.message().await.expect("read startup event") {
            if let Some(event) = response.event {
                if event.source == "docker" && event.reason == "Started" {
                    return event;
                }
                last_event = Some(event);
            }
        }
        panic!("startup watch ended before Docker Started for {sandbox_id}");
    })
    .await
    .unwrap_or_else(|_| {
        panic!("Docker startup did not finish for {sandbox_id}; last event: {last_event:?}")
    })
}
...
request = urllib.request.Request('http://{TEST_HOST}:{port}/activation')
if {authenticated}:
...
async fn configuration_activation_live_updates_keep_endpoint_credentials_paired() {
    ...
    let initial_response = request(&sandbox, upstream_a.port, true).await;
    assert!(
        initial_response.contains("STATUS 200"),
        "accepted initial policy denied its endpoint: {initial_response}"
    );
    assert!(
...
for port in [{port_a}, {port_b}]:
    for variable in ['{TOKEN_ENV}', '{TOKEN_ENV_B}']:
        if variable not in os.environ:
            continue
        request = urllib.request.Request('http://{TEST_HOST}:' + str(port) + '/activation')
...
async fn configuration_activation_rejected_image_repair_starts_once() {
    ...
    let name = format!("repair-{:012x}", rand::random::<u64>() & 0xffff_ffff_ffff);
    let mut sandbox = SandboxGuard::manage_existing(name.clone());
    ...
    let repaired_response = request(&sandbox, upstream.port, true).await;
    assert!(
        repaired_response.contains("STATUS 200"),
        "repaired policy denied its endpoint: {repaired_response}"
    );
...
async fn configuration_activation_independent_process_restarts_require_fresh_activation() {
    ...
    for role in ["supervisor", "sandbox"] {
        let name = format!("restart-{:010x}", rand::random::<u64>() & 0xff_ffff_ffff);
        let mut sandbox = SandboxGuard::manage_existing(name.clone());
        cli_ok(&[
            "sandbox",
            "create",
        ...
        let driver_started =
            wait_driver_started(original["id"].as_str().expect("sandbox ID")).await;
        let inspected = engine
            .command()
            .args(["inspect", "--format", "{{.Name}}", &boundary])
            .output()
            .expect("inspect started boundary container");
        assert!(inspected.status.success());
        let boundary_name = String::from_utf8(inspected.stdout).expect("boundary container name");
        assert_eq!(
            driver_started
                .metadata
                .get("container_name")
                .map(String::as_str),
            Some(boundary_name.trim().trim_start_matches('/')),
            "driver startup event did not match the workload container"
        );
        ...
        if ready(&settled) {
            ...
            record(
                ...
                json!({"target": role, "before": original, "driver_started": driver_started, "after": settled, "starts_before": starts, "starts_after": retained, "observed_unready": observed_unready, "workload_survived": true}),
            );
        } else {
            assert!(
                sandbox.exec(&["true"]).await.is_err(),
            ...
            // Infrastructure failures are terminal for this sandbox. Recreating
            // through the gateway authorizes a fresh runtime and initial launch.
            sandbox.cleanup().await;
            let fresh_name = format!("fresh-{:012x}", rand::random::<u64>() & 0xffff_ffff_ffff);
            sandbox = SandboxGuard::manage_existing(fresh_name.clone());
            cli_ok(&[
                "sandbox",
                "create",
                "--name",
            ...
            .await;
            let fresh = wait_accepted(&fresh_name).await;
            ...
            assert_ne!(
                fresh["configuration_admission"]["boundary_instance_id"],
                original["configuration_admission"]["boundary_instance_id"]
            );
            let new_boundary = container_id(&engine, &fresh_name);
            assert_ne!(new_boundary, boundary);
            let new_starts = durable_workload_record(&engine, &new_boundary, STARTS);
            assert_eq!(
                new_starts.lines().count(),
                1,
                "gateway recreation did not launch exactly one new main process"
            );
            record(
                "independent-process-restart",
                json!({"target": role, "before": original, "driver_started": driver_started, "after": terminal, "fresh": fresh, "starts_before": starts, "starts_after": retained, "starts_after_authorized_start": new_starts, "new_boundary_container": new_boundary, "old_boundary_container": boundary, "observed_unready": observed_unready, "workload_survived": false}),
            );
        }
        sandbox.cleanup().await;
    }
