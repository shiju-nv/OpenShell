...
# Hold workload execution when a runtime configuration is rejected.
# "retain_last_valid" requires a known accepted installation.
...
`[openshell.gateway] policy_validation_failure_mode` controls rejected effective configurations, including matching policy and provider state. The default, `fail_closed`, holds the workload, blocks new exec, keeps readiness false, closes old relays, and denies new egress until a valid configuration is accepted and released. `retain_last_valid` can keep or restore only a known accepted installation for the same boundary; it cannot resume an uncertain or mixed generation. Initial startup remains blocked when no accepted configuration exists. Provider-only updates follow the same atomic activation path.

Gateway preflight rejection leaves the active configuration unchanged because the candidate is not persisted. A runtime rejection remains visible in sandbox configuration status and can be repaired through policy or provider updates. Acceptance alone does not restore readiness: the current boundary must confirm release and the supervisor must report `activation_confirmed`. See [Repair a Rejected Configuration](/sandboxes/policies#repair-a-rejected-configuration). Restart the gateway after changing this setting so it can reload `gateway.toml` and distribute the new posture.
...
# Optional explicit container-visible host IP for aliases and trusted DNS.
# On Linux, this also selects an exact native gateway callback bind address.
# On macOS, use the host address reachable from Docker containers; it need not
# be assigned to macOS, and native callback listeners remain on loopback.
...
`host_gateway_ip` accepts an IP literal reachable from the supervisor container. On macOS, an explicit address pins both supervisor host aliases and the workload's trusted reserved-alias DNS answers; it does not request a native listener on that address. On other hosts, the same setting also selects the exact gateway callback bind address, so it must be locally bindable. When a desktop host route has no explicit address, Docker's `host-gateway` mapping remains available to the supervisor, but workload DNS for reserved host aliases stays denied because no trusted destination IP was supplied.
