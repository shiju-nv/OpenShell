...
Complete field reference for authored sandbox policy YAML and JSON. Each field is documented with its type, whether it is required, and whether it is static (locked at the first authorization to release the workload) or dynamic (updatable on a running sandbox).
...
Static fields can be repaired while configuration admission is pending or rejected and the gateway's durable `configuration_activation_authorized` value is explicitly `false`. The first authorization to release the workload locks those fields before execution may begin; a missing value or restart does not restore repair permission. After authorization, static changes require a new sandbox. Dynamic fields can be updated with `openshell policy update` for incremental merges or `openshell policy set` for full replacement when the selected compute driver supports live policy updates. Policy and provider state activate together; gateway admission alone does not establish that the workload is ready. See [Sandbox Policies](/sandboxes/policies) for repair and activation checks.

## Parsing and Representation

Authored policies and raw OPA runtime data share bounded YAML/JSON decoding. The parser rejects duplicate mapping keys and YAML merge keys, and accepts one document of at most 4 MiB. Its default limits are 64 nesting levels, 100,000 document nodes, 300,000 parser events, 4 MiB of cumulative scalar data, 100 alias expansions, a 5:1 alias-to-anchor ratio, and 10,000 entries per mapping or sequence. Reader loading retains at most the byte budget before decoding and consumes at most one extra byte to detect overflow. Regular-file loading checks metadata and also enforces the bound while reading, so file growth cannot bypass it.

### Authored Policies

Ordinary authored policy requires `version: 1`. The root and every section declared as an object or map must use named mappings. Booleans must be booleans, string fields must be strings, and path lists must contain strings. Explicit `null` values do not select omission defaults. Closed schema objects reject unknown fields.

Middleware `config`, query matcher names, GraphQL persisted-query names, and recursively nested MCP parameter names are open user-data namespaces. Their keys are data, while the surrounding policy objects and matcher operators remain closed. Schema parsing preserves nested parameter shapes; protocol-specific validation determines which matchers a runtime endpoint supports.

The proposal-risk prover and standalone containment checker use the same strict authored profile. The schema also exposes a separate containment input profile that retains unknown closed-object fields, managed `metadata`, and endpoint or allow-rule `review` annotations for a consumer support audit. Those annotations describe workflow and grant no policy authority. Runtime policies and both prover entrypoints reject them; acceptance by the audit profile does not establish containment support for a field.

An absent `filesystem_policy` resolves to `include_workdir: true`. A present `filesystem_policy: {}` resolves to `include_workdir: false`. During protobuf conversion, nonempty `ports` takes precedence over scalar `port`, empty rule names fall back to their map key, and a single effective port serializes in compact scalar form. Authored serialization omits runtime-only provenance. Because proto3 scalar fields do not preserve presence, protobuf `version: 0` means omission; canonical serialization materializes `version: 1`. Other unsupported versions and ports above 65535 are rejected.

### Raw OPA Runtime Data

An operator-supplied OPA data document has a separate runtime representation. It may omit `version` and retain unrelated application data at the root. Governed policy sections still use the canonical scalar types, defaults, and closed-object rules. This raw representation preserves runtime endpoint provenance, resolved binary entries, lowered protocol settings, and explicit matcher objects. It does not turn those fields into authored-policy options, derive provenance for unstamped inputs, or let input select the trusted binary-identity mode. The loader validates the supplied data document; custom Rego code can produce additional values during evaluation.

For query parameters and supported MCP parameter matchers, the OPA loader normalizes nonempty strings to explicit `glob` objects after validation. For example, `query: { status: "active*" }` becomes `query: { status: { glob: "active*" } }` in runtime endpoint data. This applies to allow and deny rules and MCP `tool` aliases. Matchers supported by YAML and protobuf use the same runtime representation; `any` matchers retain their listed values. Raw OPA data also accepts empty scalar query matchers, which match empty query values and remain unchanged. An empty protobuf glob cannot represent that OPA-only matcher.

## Policy Load Diagnostics

When the supervisor's OPA engine rejects a load or reload from a file, reader, string, or protobuf, its returned error identifies the validation category without copying policy or middleware names, hostnames, paths, values, source text, or the original error chain.

Each message contains at most eight error items and 512 UTF-8 bytes, including its heading, separators, and any `additional violations omitted` marker. Categories identify which part of the submitted input to inspect:
...
- Shared raw-schema validation reports a fixed schema location and category for malformed values or closed-object fields.
...
- Parser errors report a fixed category and numeric line and column when available. File I/O, Rego loading, and internal policy-data failures return fixed messages.

A rejected engine reload preserves its previous policy and generation. The supervisor separately applies `policy_validation_failure_mode`. In a managed control/boundary sandbox, the default `fail_closed` posture holds workload execution and readiness, denies new egress, and invalidates old relays until matching policy and provider state activate. `retain_last_valid` permits only a verified previous accepted configuration to remain active; it cannot release an uncertain mixture or an initial sandbox without such a configuration. See [Gateway Configuration](/reference/gateway-config) for that setting and [Sandbox Policies](/sandboxes/policies) for repair.

These bounds apply to returned OPA load errors. Accepted-policy warnings, runtime request diagnostics, and gateway-authored policy parser messages have separate reporting contracts.
...
Policies that violate these constraints are rejected with `INVALID_ARGUMENT` at creation or update time. A malformed raw OPA filesystem section rejects the engine load; a failed reload preserves the installed engine. Managed runtime recovery follows the configuration admission and rejection posture described above.
...
| `hard_requirement` | Aborts sandbox startup. | Aborts sandbox startup. | Aborts privileged preparation; skips the path when the boundary opens paths as the workload user. | Aborts sandbox startup. |
...
`hard_requirement` makes unavailable kernel support, empty configured path sets, and an entirely inaccessible path set fatal. During privileged preparation, an individual path-open failure also aborts startup.

The capability-free boundary opens policy paths as the workload user. A path that this user cannot open is already unavailable to the workload, so the boundary skips that path and applies the remaining rules. The other `hard_requirement` failures still abort startup. The boundary also installs a mandatory filesystem baseline protecting its private runtime files; failure to install that baseline aborts startup independently of the optional policy's compatibility setting.
