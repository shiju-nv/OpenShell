...
The policy prover uses the same strict authored profile. The containment profile for maximum-policy tooling retains unknown closed-object fields, managed `metadata`, and endpoint or allow-rule `review` annotations for a support audit. Those annotations describe workflow and grant no policy authority. Ordinary runtime policies reject them, and schema acceptance alone does not establish containment support for a field.
