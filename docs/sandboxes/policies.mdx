...
While execution is held, requested process signals and timeout termination wait until the configuration is released. You can still stop the sandbox during a hold.
...
The supervisor retries the repaired configuration. Successful repair permits one initial workload launch; repeated acknowledgements cannot launch it again. Wait for `activation_confirmed: true` and the current sandbox's `Ready` condition before using it. Policy history becomes `loaded` only after the gateway accepts the matching final activation report. The displayed `policy_source` is the scope `sandbox` or `global`, not the image or default origin of the policy.
