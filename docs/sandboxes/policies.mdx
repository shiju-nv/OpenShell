...
OpenShell uses an exact-pinned `tower-mcp-types` inspection API for structural JSON-RPC and selected-profile MCP method, direction, message-kind, and typed-parameter checks. It rejects duplicate JSON object keys before constructing the value passed to that API, rejects methods known to Tower but unavailable in the selected revision, and preserves unknown methods as explicitly authorizable extensions. OpenShell does not treat the client's `initialize.params.protocolVersion` as the selected revision, inspect the server's initialization response, or bind `MCP-Session-Id`. The version check is stateless and applies independently to each later request, including GET and DELETE.
