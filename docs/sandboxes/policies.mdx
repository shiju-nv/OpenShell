...
A policy has static sections `filesystem_policy`, `landlock`, and `process` that become locked when the gateway first authorizes workload release. Dynamic `network_policies` and `network_middlewares` sections can be updated on a running sandbox.

```yaml wordWrap showLineNumbers={false}
version: 1

# Static: locked before first workload release. Read-only and writable paths.
filesystem_policy:
...
A rejected initial configuration can be repaired before the gateway authorizes its first workload release. After that authorization, changing static sections requires a new sandbox, even if the workload has not yet started or a later acknowledgement fails. See [Repair a Rejected Configuration](#repair-a-rejected-configuration).

Dynamic sections can be updated with `openshell policy update` for incremental merges or `openshell policy set` for full replacement. The supervisor prepares policy and matching providers together, holds workload execution and readiness during installation, and resumes only after the complete configuration is accepted and released. Provider-only changes follow the same activation path.
...
The first TCP endpoint is a deliberate exception to ordinary dynamic network policy updates. A sandbox created without any `protocol: tcp` endpoint does not install the DNS and transparent-capture substrate. A hot reload that introduces the first TCP endpoint is rejected atomically; the configured validation failure mode determines whether the workload stays blocked or retains a known accepted configuration. Recreate the sandbox with a TCP endpoint to install the substrate. A sandbox that started with TCP support can remove and later re-add TCP endpoints through normal policy reloads.
...
The supervisor starts the workload after the repaired configuration passes validation and the matching installation receives release authorization. You can replace static policy fields only while admission is pending or rejected and durable state explicitly records that release has never been authorized. The first authorization locks static fields permanently, including while a restart is pending or rejected. Images without an embedded policy use the restrictive baseline and gain network access only from operator-selected configuration. Explicit user/global policy precedence remains unchanged. Gateway authorization or compatibility errors, exhausted connection retries, and unavailable sandbox boundary connections terminate startup rather than waiting for policy repair.
...
In `fail_closed` mode, OpenShell holds the workload, blocks new exec, and keeps readiness false. The supervisor denies new egress and closes connections pinned to the previous policy. A later valid configuration resumes execution only after matching policy and provider installation, gateway acceptance, and confirmed boundary release.
...
In `retain_last_valid` mode, the rejected candidate remains inactive. OpenShell can keep or restore the previous accepted configuration only when its installation is still known for the same workload boundary. This mode cannot resume an uncertain or mixed installation, and initial startup remains blocked when no accepted configuration exists. Restart the gateway after changing `gateway.toml`; connected supervisors receive the configured posture from the restarted gateway. Individual sandboxes cannot override it.
...
### Repair a Rejected Configuration

An invalid image policy, unresolved provider binding, or unavailable matching provider environment can leave a newly created sandbox waiting for configuration repair. The workload stays blocked, and the sandbox remains available through the gateway for inspection and policy/provider changes. When neither user nor image policy is available, OpenShell selects restrictive defaults; absence of image policy does not grant unrestricted network access.

Inspect the desired configuration and admission error:

```shell
openshell sandbox get my-sandbox --output json
openshell provider list
openshell provider get my-provider
```

`configuration_desired` describes the candidate delivered to the supervisor. `configuration_admission` describes its accepted or rejected installation state. An `accepted` state with `activation_confirmed: false` is still waiting for boundary release confirmation. Under `retain_last_valid`, an older accepted installation may remain usable while a newer desired candidate reports an error.

Correct the base policy and any referenced provider credentials, profile, or attachment. Use [Manage Providers](/sandboxes/manage-providers) for credential repair and [Attach and Detach Providers](/providers/profiles#attach-and-detach-providers) for attachment changes. Submit a corrected policy:
...
Before first release authorization, a pending or rejected sandbox with an explicit never-authorized marker can accept static policy corrections as well as network changes. The gateway consumes this permission before the workload may run. A missing marker, a failed acknowledgement, or a supervisor restart does not reopen it. After authorization, create a new sandbox to change static policy.

The supervisor retries the repaired configuration within the provisioning repair window. Successful repair permits one initial workload launch; repeated acknowledgements cannot launch it again. If the window has expired, wait for cleanup and explicitly start the sandbox as described under [Validation Failures](#validation-failures). Wait for `activation_confirmed: true` and the current sandbox's `Ready` condition before using it. Policy history becomes `loaded` only after the gateway accepts the matching final activation report. The displayed `policy_source` is the scope `sandbox` or `global`, not the image or default origin of the policy.
