...
### Inspect Attached Providers

Inspect the provider records attached to a sandbox:

```shell
openshell sandbox provider list my-sandbox --output json
```

JSON and YAML output contain an array of providers. Each entry includes the immutable provider `id`, `name`, `workspace`, `resource_version`, `type`, and `profile_workspace`, plus sorted `credential_keys` and `config_keys`. An empty `profile_workspace` selects the platform profile. Credential values, storage handles, and configuration values are omitted. Compare provider IDs when checking for replacement under the same name; `resource_version` describes the provider record and does not establish that a supervisor applied it.
