...
Get detailed information about a sandbox. The policy view shows the selected desired policy, its scope (`sandbox` or `global`), and its revision. It does not by itself prove that the workload installed that configuration:
...
Structured details separate `configuration_desired` from `configuration_admission`. The former names the delivered candidate; the latter records installation state and `activation_confirmed`. An accepted candidate may still be waiting for release confirmation. A desired rejection can also coexist with an older accepted configuration when the gateway permits retaining it. Use the configuration error and [Repair a Rejected Configuration](/sandboxes/policies#repair-a-rejected-configuration) to correct policy or providers without restarting a blocked initial workload.

The repair wait applies after the gateway acknowledges a configuration rejection. Startup calls that receive no response or repeatedly fail terminate after bounded retries. For those failures, inspect gateway connectivity and authentication before retrying startup.

Print only the desired effective policy YAML for scripting, without activation metadata:
...
Stop and start are idempotent. You can also start a retained `Completed` or `Error/MainProcessFailed` sandbox to launch a fresh instance of its canonical command. Starting a fresh instance creates a new driver runtime generation, invalidates prior SSH sessions, and requires fresh configuration activation. Delete an inactive sandbox normally when you no longer need its retained state.

If the control supervisor or container exits, first inspect `openshell sandbox get my-sandbox --output json`. For phase `Error` with a `Ready=False` condition whose reason is exactly `ControlSupervisorExited` or `ContainerExited`, explicitly stop the sandbox before starting it again. These conditions do not replace the driver's stop confirmation. The stop command waits for driver-confirmed `Stopped`; direct start from either failure state is refused. Start only after stop succeeds:

```shell
if openshell sandbox stop my-sandbox
    openshell sandbox start my-sandbox
end
openshell sandbox get my-sandbox --output json
```

The new runtime generation must complete fresh configuration activation before readiness returns. This recovery path requires one of these exact failure conditions; other infrastructure errors need diagnosis of their reported reason and do not become restartable through this procedure.
...
Editing configuration after expiry does not restart compute. A retry gets a new 300-second window, while static-policy restrictions from any previous release authorization remain in force. Timed-out records are retained even for ephemeral creates; use `sandbox delete` when you no longer need the diagnostic or stored state.
...
| Ready        | The current control session is connected and the accepted configuration has confirmed activation. You can connect, execute commands, sync files, and view logs. |
...
| Starting     | Compute is starting. Access requires a fresh supervisor session and matching configuration activation. |
...
Compute readiness and a connected supervisor session are insufficient to make a sandbox usable. The gateway also requires an accepted configuration with `activation_confirmed: true` for the current runtime. A disconnected supervisor produces `SupervisorNotConnected`; incomplete or rejected activation produces configuration conditions such as `ConfigurationPending` or `ConfigurationInvalid`. Inspect `conditions`, `configuration_desired`, and `configuration_admission` with `openshell sandbox get my-sandbox --output json`.

After a connection loss, OpenShell holds the workload while the supervisor recovers. A replacement supervisor must register with the gateway, attach with a current signed grant, and revalidate the configuration. Boundary confirmation alone cannot resume execution. If recovery expires or the boundary process is destroyed, the old runtime generation is terminal; launching again requires a new driver runtime generation. Wait for `Ready` before reconnecting or executing commands.
