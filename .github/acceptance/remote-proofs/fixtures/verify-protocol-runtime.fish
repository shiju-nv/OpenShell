...
set -l evidence "$fixtures/runs/protocol-runtime-"(date -u +%Y%m%dT%H%M%SZ)
...
or exit 2
set -l tests \
    configuration::tests::configuration_activation_protocol_startup_matrix \
    configuration::tests::configuration_activation_protocol_update_matrix \
    configuration::tests::configuration_activation_protocol_authored_empty_is_not_proto_omission
for test_name in $tests
    set -l label (string replace -a :: - $test_name)
    printf 'Running actual coordinator proof %s\n' $test_name
    set -l test_args --exact $test_name --nocapture
    if test $test_name != configuration::tests::configuration_activation_protocol_authored_empty_is_not_proto_omission
        set test_args --ignored $test_args
    end
...
uv run --no-project python "$fixtures/verify-protocol-observations.py" "$evidence"
or exit $status
printf 'Protocol coordinator slice passed. Gateway delivery is controlled; probes inspect installed decisions and credential rewrite.\n'
