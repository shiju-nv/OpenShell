...
pub async fn run_sandbox(
    ...
    if let Some(receiver) = endpoint_status_rx {
        let endpoint = endpoint.clone();
        let sandbox_id = id.clone();
        let credentials = extension_credentials.clone();
        // Endpoint reporting is passive and cannot block admission or traffic.
        // Its accepted-session watch fences reports across access-plane reconnects.
        tokio::spawn(async move {
            match openshell_core::grpc_client::CachedOpenShellClient::connect_with_credentials(
                &endpoint,
                credentials,
            )
            .await
            {
                ...
                Err(error) => warn!(error = %error, "Endpoint status reporter could not connect"),
            }
        });
    }

    // Publish the workspace only after complete configuration activation so
    // flush tasks and the policy.local API use the accepted generation.
