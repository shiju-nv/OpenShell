...
impl AuthMetadataFixture {
    async fn new() -> Self {
        ...
        let sandbox = state
            ...
            .unwrap();
        let identity =
            PersistedSandboxIdentity::read(&sandbox.metadata.as_ref().unwrap().annotations)
                .unwrap();
        let authority = state
    ...
    async fn persisted_state(
        ...
        (
            ...
            self.state
                ...
                .unwrap(),
            settings_snapshot(
                load_sandbox_settings(self.state.store.as_ref(), "default", "activation")
                    .await
                    .unwrap(),
            ),
            settings_snapshot(
...
#[tokio::test]
async fn configuration_activation_auth_metadata_rejects_authority_annotations_without_mutation() {
    ...
    let changed = [
        (
            ...
            CONFIGURATION_TEST_GATEWAY_TOKEN_ID.to_string(),
        ),
        (
...
#[tokio::test]
async fn configuration_activation_auth_metadata_rejects_setting_before_mutation() {
    let fixture = AuthMetadataFixture::new().await;
    let before = fixture.persisted_state().await;
    let request = fixture.request(
        UpdateConfigRequest {
...
#[tokio::test]
async fn configuration_activation_auth_metadata_preserves_ordinary_annotations() {
    for sandbox_caller in [true, false] {
        let fixture = AuthMetadataFixture::new().await;
        let before = fixture.persisted_state().await;
        ...
        assert_eq!(metadata.annotations["example.com/review-note"], "retained");
        assert_eq!(response.annotations["example.com/review-note"], "retained");
