...
/// Per-request synchronization for policy commit ordering regressions.
#[cfg(test)]
#[derive(Clone)]
struct PolicyCommitPause {
...
pub(super) async fn handle_get_sandbox_config(
    ...
    let mut desired = openshell_core::proto::SandboxConfigurationSnapshot {
        ...
        endpoint_configuration: response
            .policy
            .as_ref()
            .map(endpoint_status::configuration_endpoint_inventory),
        ..Default::default()
    };
...
async fn handle_update_config_inner(
    ...
    if let Some(pause) = commit_pause {
        ...
        pause
            ...
            .acquire()
            .await
            .expect("policy commit test resumes its request")
            .forget();
    }
    ...
    // The atomic write is this request's only commit point. A later writer may
    // already have replaced it, so return our committed revision without
    // appending the stale request payload over a newer policy or static repair.
    info!(
        sandbox_id = %sandbox_id,
    ...
    emit_full_policy_update_success(sandbox_caller, committed_version);

    Ok(update_config_response(
        u32::try_from(committed_version).unwrap_or(0),
        hash,
        0,
        false,
        response_annotations,
...
pub fn authorize_configuration_identity(
    principal: &Principal,
    sandbox: &Sandbox,
    ...
    // A canonical-process exit retains the already authorized runtime for
    // output delivery and policy-governed exec. Its current control must still
    // poll and repair rejected configurations; confirmation may be held then.
    // Infrastructure errors without an exit result confer no retained access.
    let retained_terminal = matches!(phase, Some(SandboxPhase::Completed | SandboxPhase::Error))
            ...
            status.exit_code.is_some()
                ...
                && status
                    ...
                    .is_some_and(|admission| {
                        admission.runtime_generation == identity.runtime_generation.as_str()
                            && !admission.instance_id.is_empty()
                            && status.main_process_instance_id == admission.instance_id
                            && !admission.boundary_instance_id.is_empty()
                            && !admission.boundary_session_id.is_empty()
                            && admission.registration_revision != 0
                    })
        });
    // Gateway bearer rotation preserves runtime generation and auth epoch.
    // Recheck its token ID here so prior authentication cannot survive rotation
    // while waiting to register a control session or mutate configuration.
    if principal.sandbox_id != sandbox.object_id()
        ...
        || !(retained_terminal
            || matches!(
                phase,
                Some(SandboxPhase::Provisioning | SandboxPhase::Starting | SandboxPhase::Ready)
            ))
    {
...
// Length-prefix every component so distinct field boundaries cannot produce
// the same gateway configuration fingerprint encoding.
...
pub const CONFIGURATION_REGISTRATION_HISTORY: &str =
...
pub(super) async fn handle_report_sandbox_configuration(
    ...
    if registration_changed
        && matches!(
            openshell_core::proto::SandboxPhase::try_from(sandbox.phase()),
            Ok(openshell_core::proto::SandboxPhase::Completed
                | openshell_core::proto::SandboxPhase::Error)
        )
    {
        // Terminal access belongs only to the control/boundary tuple that
        // already ran this main process. A replacement requires a new launch.
        return Err(Status::failed_precondition(
            "terminal runtime cannot replace its control or boundary registration",
        ));
    }
    let mut admission = reported_admission;
    // Endpoint identity is derived by the gateway at delivery, never supplied
    // by the runtime that reports traffic observations.
    admission.endpoint_configuration = None;
    let mut endpoint_reset = None;
    ...
    if reported == ConfigurationAdmissionState::Pending {
        ...
        if reported == ConfigurationAdmissionState::Accepted {
            ...
            if admission.activation_confirmed {
                endpoint_reset =
                    endpoint_status::endpoint_status_reset_for_configuration(current, desired);
                admission
                    .endpoint_configuration
                    .clone_from(&desired.endpoint_configuration);
            } else if endpoint_status::endpoint_configuration_matches(current, desired) {
                // Preserve same-configuration observation provenance while held.
                // A changed tuple has no accepted inventory until confirmation.
                admission.endpoint_configuration =
                    current.and_then(|current| current.endpoint_configuration.clone());
            }
        } else {
...
mod tests {
    ...
    /// Gateway token shared by exact launch principals in activation fixtures.
    pub(super) const CONFIGURATION_TEST_GATEWAY_TOKEN_ID: uuid::Uuid =
        uuid::Uuid::from_u128(0x870e_fbe4_cd48_4344_9000_facb_08a0_d015);
        ...
        request
            ...
            .insert(Principal::Sandbox(SandboxPrincipal {
                ...
                source: SandboxIdentitySource::LaunchSession {
                    ...
                    token_id: CONFIGURATION_TEST_GATEWAY_TOKEN_ID,
                },
                trust_domain: Some("openshell".to_string()),
            }));
        request
    ...
    #[tokio::test]
    async fn configuration_activation_endpoint_inventory_uses_issued_policy_after_global_change() {
        use openshell_core::proto::{
            ConfigurationAdmissionState as Admission, EndpointObservation, EndpointResult,
            EndpointStatus, ReportEndpointStatusRequest, SandboxEndpointConfiguration,
        };
        ...
        state
            ...
            .await
            .unwrap();
        let issued = configuration_activation_poll(&state, &id, &admission.instance_id).await;
        let inventory =
            endpoint_status::configuration_endpoint_inventory(issued.policy.as_ref().unwrap());
        assert_eq!(inventory.endpoints.len(), 1);
        admission.delivery_revision = issued.configuration_delivery_revision;
        admission.configuration_snapshot = issued.configuration_snapshot;
        admission.policy_version = issued.version;
        admission.policy_hash = issued.policy_hash;
        admission.policy_source = issued.policy_source;
        admission.config_revision = issued.config_revision;
        admission.provider_env_revision = issued.provider_env_revision;
        ...
        .await
        .unwrap();
        let observed = handle_get_sandbox_config(
            &state,
            with_sandbox(
                Request::new(GetSandboxConfigRequest {
                    sandbox_id: id.clone(),
        ...
        .unwrap()
        .into_inner();
        assert_ne!(observed.policy_hash, admission.policy_hash);

        // The runtime cannot replace gateway-owned endpoint identity, even
        // when its complete installation receipt is otherwise valid.
        admission.endpoint_configuration = Some(SandboxEndpointConfiguration {
            endpoints: vec![EndpointStatus {
                endpoint_id: "forged".to_string(),
                ..Default::default()
            }],
            credentialed_endpoint_ids: vec!["forged".to_string()],
        });
        ...
        let accepted = state
            ...
            .unwrap()
            .unwrap();
        let status = accepted.status.unwrap();
        assert_eq!(status.endpoint_statuses, inventory.endpoints);
        assert_eq!(
            status
                .configuration_admission
                .unwrap()
        ...
        state.supervisor_sessions.register(
            id.clone(),
            "accepted-session".to_string(),
            session_tx,
            shutdown_tx,
        );
        reset_endpoint_status_for_supervisor_session(&state, &id, "accepted-session")
            .await
            .unwrap();
        assert!(
            state
                .supervisor_sessions
                .initialize_endpoint_status_authority(&id, "accepted-session")
        );
        let endpoint_id = inventory.endpoints[0].endpoint_id.clone();
        let report = ReportEndpointStatusRequest {
            ...
            supervisor_session_id: "accepted-session".to_string(),
            report_sequence: 1,
        };
        handle_report_endpoint_status(&state, with_sandbox(Request::new(report), &id))
            .await
            .unwrap();
        let before_retry = state
            ...
            .endpoint_statuses;
        assert_eq!(
            before_retry[0].last_result,
            i32::from(EndpointResult::HttpResponseReceived)
        );
        assert!(!before_retry[0].last_reported_at.is_empty());
        handle_report_sandbox_configuration(
            &state,
    ...
    #[tokio::test]
    async fn configuration_activation_committed_image_sync_cannot_undo_static_repair() {
        ...
        let baseline = state
            ...
            .unwrap()
            .unwrap();
        let mut image = baseline.spec.as_ref().unwrap().policy.clone().unwrap();
        image
            .filesystem
            .as_mut()
            .unwrap()
            .read_only
            .push("/image-enrichment".to_string());
        let image_hash = deterministic_policy_hash(&image);
        assert_ne!(
            image_hash, admission.policy_hash,
            "the image sync must not deduplicate"
        );
        let entered = Arc::new(tokio::sync::Notify::new());
        let resume = Arc::new(tokio::sync::Semaphore::new(0));
        let mut image_request = with_sandbox(
            Request::new(UpdateConfigRequest {
                name: "activation".to_string(),
                workspace_scope: Some(openshell_core::proto::workspace_selector("default")),
                policy: Some(image.clone()),
                ..Default::default()
            }),
            &id,
        );
        image_request.extensions_mut().insert(PolicyCommitPause {
            entered: entered.clone(),
            resume: resume.clone(),
        });
        let image_state = state.clone();
        let image_task =
            tokio::spawn(async move { handle_update_config(&image_state, image_request).await });
        tokio::time::timeout(std::time::Duration::from_secs(5), entered.notified())
            .await
            .expect("image sync committed its first history revision");
        let image_record = state.store.get_latest_policy(&id).await.unwrap().unwrap();
        assert_eq!(image_record.policy_hash, image_hash);
        assert_eq!(
            state
                .store
                .get_message::<Sandbox>(&id)
                ...
                .unwrap(),
            baseline,
            "history-only image enrichment leaves the observed sandbox version unchanged"
        );

        let mut repair = image;
        repair.filesystem.as_mut().unwrap().include_workdir =
            !repair.filesystem.as_ref().unwrap().include_workdir;
        let repaired_response = handle_update_config(
            &state,
            with_user(Request::new(UpdateConfigRequest {
                name: "activation".to_string(),
                workspace_scope: Some(openshell_core::proto::workspace_selector("default")),
        ...
        let repaired = state
            ...
            .unwrap()
            .unwrap();
        let repaired_record = state.store.get_latest_policy(&id).await.unwrap().unwrap();
        assert_eq!(
            repaired.spec.as_ref().unwrap().policy.as_ref(),
            Some(&repair)
        );
        assert_eq!(repaired_record.version, image_record.version + 1);
        assert_eq!(
            u32::try_from(repaired_record.version).unwrap(),
            repaired_response.version
        );

        resume.add_permits(1);
        let image_response = tokio::time::timeout(std::time::Duration::from_secs(5), image_task)
            .await
            .expect("committed image sync returns after repair")
            .unwrap()
            .unwrap()
            .into_inner();
        assert_eq!(
            image_response.version,
            u32::try_from(image_record.version).unwrap()
        );
        assert_eq!(image_response.policy_hash, image_hash);
        assert_eq!(
            state
                .store
                .get_message::<Sandbox>(&id)
        ...
        assert_eq!(delivered.version, repaired_response.version);
        assert_eq!(delivered.policy_hash, repaired_response.policy_hash);
        assert_eq!(delivered.policy.unwrap().filesystem, repair.filesystem);
    }

    async fn retained_terminal_configuration_fixture(
        exit_code: i32,
    ) -> (
        Arc<ServerState>,
        String,
        ...
        state
            .compute
            .report_main_process_exit(&id, &admission.instance_id, exit_code)
            .await
            .unwrap();
        (state, id, admission)
    }
    ...
    #[tokio::test]
    async fn configuration_activation_terminal_current_runtime_polls_renews_and_repairs() {
        use openshell_core::proto::{ConfigurationAdmissionState as Admission, SandboxPhase};
        for exit_code in [0, 7] {
            let (state, id, mut admission) =
                retained_terminal_configuration_fixture(exit_code).await;
            ...
            for rejected in [false, true] {
                ...
                let renewed = handle_report_sandbox_configuration(
                    &state,
                    configuration_activation_report(&id, pending, "", ""),
                )
                .await
                .unwrap()
                .into_inner();
                ...
                // A held terminal runtime can accept its repaired installation;
                // the final acknowledgement never revives its lifecycle phase.
                admission.state = Admission::Accepted.into();
                admission.activation_confirmed = false;
                handle_report_sandbox_configuration(
                    &state,
    ...
    #[tokio::test]
    async fn configuration_activation_terminal_runtime_rejects_replacement_and_unlaunched_identity()
    {
        use openshell_core::proto::ConfigurationAdmissionState as Admission;
        for exit_code in [0, 7] {
            let (state, id, admission) = retained_terminal_configuration_fixture(exit_code).await;
            for coordinate in ["control", "boundary", "session"] {
                let before = state
                    .store
                    .get_message::<Sandbox>(&id)
                    .await
                ...
                match coordinate {
                    "control" => pending.instance_id = replacement,
                    "boundary" => pending.boundary_instance_id = replacement,
                    "session" => pending.boundary_session_id = replacement,
                ...
                .await
                .unwrap_err();
                assert_eq!(error.code(), Code::FailedPrecondition, "{coordinate}");
                assert_eq!(
                    state
                        .store
                        .get_message::<Sandbox>(&id)
            ...
            for invalid in [
                ...
                state
                    ...
                    .update_message_cas::<Sandbox, _>(&id, 0, |sandbox| {
                        ...
                        match invalid {
                            ...
                            "unknown-authorization" => {
                                status.configuration_activation_authorized = None;
                            }
                            "no-main-exit" => status.exit_code = None,
                            "other-main" => {
                                status.main_process_instance_id = uuid::Uuid::new_v4().to_string();
                            }
                            "other-runtime" => {
                            ...
                            "other-epoch" => {
                                sandbox.metadata.as_mut().unwrap().annotations.insert(
                                    crate::auth::sandbox_session::AUTH_EPOCH_ANNOTATION.to_string(),
                                    "2".to_string(),
                                );
                            }
                            _ => unreachable!("fixed invalid identity fixture"),
                        }
                    })
                    .await
                    .unwrap();
                let invalid_record = state
                    .store
                    .get_message::<Sandbox>(&id)
                    .await
                    .unwrap()
                ...
                let error = handle_get_sandbox_config(
                    ...
                    configuration_activation_request(
                        Request::new(GetSandboxConfigRequest {
                            sandbox_id: id.clone(),
                            configuration_instance_id: admission.instance_id.clone(),
                        }),
                        &id,
                    ),
                )
                .await
                .unwrap_err();
                assert_eq!(error.code(), Code::Unauthenticated, "{invalid}");
                let mut pending = admission.clone();
                pending.state = Admission::Pending.into();
                assert_eq!(
                    handle_report_sandbox_configuration(
                ...
                state.store.put_message(&before).await.unwrap();
            }
        }
    }
    ...
    #[tokio::test]
    async fn configuration_activation_compute_readiness_final_report_publishes_ready_with_loaded_policy()
     {
        use openshell_core::proto::{ConfigurationAdmissionState as Admission, SandboxPhase};
        let (state, id, mut admission) = configuration_activation_fixture().await;
        state
        ...
        .await
        .unwrap();
        // The same final report that publishes loaded policy history must
        // remove its readiness gate without another driver/session event.
        let activated = state
            .store
            .get_message::<Sandbox>(&id)
            .await
            .unwrap()
            .unwrap();
        assert_eq!(activated.phase(), SandboxPhase::Ready as i32);
        assert_eq!(activated.current_policy_version(), admission.policy_version);
        assert!(
            activated
                .status
                .as_ref()
    ...
    #[tokio::test]
    async fn configuration_activation_signed_jwt_is_rechecked_after_same_epoch_token_rotation() {
        use crate::auth::authenticator::Authenticator;
        let (state, id, admission) = configuration_activation_fixture().await;
        let sandbox = state
            .store
        ...
        let rotated = crate::auth::sandbox_session::rotate_gateway_token(
            state.store.as_ref(),
            &authenticated,
            &successor,
        )
        .await
        .unwrap();
        assert_eq!(rotated.runtime_generation, identity.runtime_generation);
        assert_eq!(rotated.auth_epoch, identity.auth_epoch);
        assert_ne!(rotated.gateway_token_id, identity.gateway_token_id);
        let before = state
            .store
            .get_message::<Sandbox>(&id)
            .await
        ...
        assert_eq!(
            handle_report_sandbox_configuration(&state, request)
                ...
                .code(),
            Code::Unauthenticated,
            "an authenticated request cannot mutate after its gateway token rotates in the same runtime epoch"
        );
        assert_eq!(
            state
                .store
