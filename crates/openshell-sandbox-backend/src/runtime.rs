...
use openshell_isolation_interface::contract::{
    ...
    PreparedBoundaryConfiguration, ProcessAttachment, ProviderEnvironmentInstallation,
...
impl ReadyBoundary for RemoteReady {
    ...
    async fn start_agent(self: Box<Self>) -> Result<Box<dyn RunningBoundary>, BackendError> {
        ...
        let current = self.provider_credentials.snapshot();
        let activation = self.client.active_configuration(current.revision)?;
        if activation.provider_env_installation_id != current.installation_id {
            return Err(BackendError::Denied(
                "provider snapshot does not match active configuration".to_string(),
            ));
        }
        let response = self
            .client
...
impl BoundaryExec for RemoteExec {
    ...
    async fn synchronize_provider_environment(
        ...
        // Reporting consumes the exact accepted installation and proves peer
        // liveness. It cannot install credentials ahead of configuration release.
        let current = self.provider_credentials.snapshot();
        let activation = self.client.active_configuration(current.revision)?;
        if activation.provider_env_installation_id != current.installation_id {
            return Err(BackendError::Denied(
        ...
        self.client.require_active(&activation)?;
        if !snapshot.active
            || snapshot.installed.as_ref() != Some(&activation.configuration)
            || snapshot.publication_generation != activation.publication_generation
            || snapshot.provider_env_installation_id.as_ref() != Some(&current.installation_id)
        {
            return Err(BackendError::Denied(
                "boundary did not acknowledge the active provider installation".to_string(),
            ));
        }
        let latest = self.provider_credentials.snapshot();
        if latest.installation_id != current.installation_id || latest.revision != current.revision
        {
            return Err(BackendError::Denied(
                "provider snapshot changed during boundary acknowledgement".to_string(),
            ));
        }
        Ok(ProviderEnvironmentInstallation {
...
impl NetworkMediationSource for RemoteNetworkMediation {
    async fn accept_tcp(&self) -> Result<PendingTcpOpen, BackendError> {
        let (stream, response) = self
            .client
            .accept_mediation(|epoch| {
                self.client
                    .call_wait_stream(Request::AcceptNetwork, Some(epoch))
            })
            .await?;
...
impl ClientMediationSession {
    fn start(stream: BoundaryDuplexStream, activation_epoch: u64) -> Arc<Self> {
...
impl BoundaryClient {
    ...
    async fn replay_attachment(
        ...
        // Every replay step uses the same captured bearer; a concurrent renewal
        // must not publish a fingerprint for a credential that was never confirmed.
        let discovery = Self::prepare_request(Request::DescribeWorkload {
            supervisor_instance_id: self.supervisor_instance_id,
            resource_claims: self.runtime_descriptor.resource_claims.clone(),
        })?;
        let response = self
            .exchange_on_channel(channel.clone(), &discovery, credential)
            .await?;
        let Response::WorkloadDescribed { bootstrap } = response else {
            return Err(unexpected_response("workload_described", &response));
        };
        ...
        let response = self
            .exchange_on_channel(channel.clone(), attach, credential)
            .await?;
        let Response::Attached { snapshot } = response else {
            return Err(unexpected_response("attached", &response));
        };
        ...
        if let Some(confirm) = confirm {
            let response = self
                .exchange_on_channel(channel, confirm, credential)
                .await?;
            let Response::Confirmed { evidence } = response else {
    ...
    async fn call_idempotent(&self, request: Request) -> Result<Response, BackendError> {
        ...
        // Discovery precedes attachment, so it must tolerate the same trusted
        // image bootstrap delay. Live control operations keep their shorter bound.
        let timeout = if remember_attach || matches!(request, Request::DescribeWorkload { .. }) {
    ...
    async fn call_wait(&self, request: Request) -> Result<Response, BackendError> {
        let (_, response) = self.call_wait_stream(request, None).await?;
        Ok(response)
    }

    /// Idle accepts have no operation deadline. Only transport failure starts
    /// recovery; activation loss returns to the caller's fresh-release wait.
    ...
    async fn ensure_current_credential_connection(&self) -> Result<(), BackendError> {
        ...
        self.replay_attachment(channel.clone(), &attach, Some(&confirm), &credential)
            .await?;
    ...
    /// Replace only the physical connection observed before the failed attempt.
    /// A late failure cannot retire another caller's recovered connection.
    async fn recover_after_unavailable(
        ...
        *self.mediation.lock().await = None;
        ...
        let credential = BoundaryCredential::capture(&self.sandbox_bearer)?;
        let channel = self.build_grpc_channel().await?;
        self.replay_attachment(channel.clone(), &attach, confirm.as_ref(), &credential)
            .await?;
        *self.grpc_channel.lock().await = Some(CachedGrpcChannel {
            credential_epoch: credential.epoch,
            bearer_fingerprint: credential.fingerprint,
    ...
    async fn healthy_mediation_session(
        &self,
        activation_epoch: u64,
    ) -> Option<Arc<ClientMediationSession>> {
        self.mediation
            .lock()
            .await
            .as_ref()
            .filter(|session| session.is_healthy() && session.activation_epoch == activation_epoch)
            .cloned()
    }
    ...
    async fn open_mediation_session(
        ...
        Ok(ClientMediationSession::start(stream, activation_epoch))
    }
...
impl BoundaryConfiguration for BoundaryClient {
    ...
    async fn snapshot(&self) -> Result<BoundaryConfigurationSnapshot, BackendError> {
            ...
            let installation_id = snapshot
                ...
                .ok_or_else(|| {
                    BackendError::Confirm(
                        "installed boundary snapshot has no provider identity".to_string(),
                    )
                })?;
            validate_installation_id(installation_id)?;
            if snapshot.publication_generation == 0 {
                return Err(BackendError::Confirm(
                    "installed boundary snapshot has no publication generation".to_string(),
                ));
            }
        } else if snapshot.publication_generation != 0
            || snapshot.provider_env_installation_id.is_some()
        {
            return Err(BackendError::Confirm(
                "uninstalled boundary snapshot carries provider publication evidence".to_string(),
            ));
        ...
        let released_tuple_changed = self
            ...
            .is_some_and(|activated| {
                snapshot.active
                    && (snapshot.installed.as_ref() != Some(&activated.configuration)
                        || snapshot.publication_generation != activated.publication_generation
                        || snapshot.provider_env_installation_id.as_ref()
                            != Some(&activated.provider_env_installation_id))
            });
    ...
    async fn commit(
        ...
        validate_prepared_receipt(
            ...
            &prepared.provider_env_installation_id,
        )?;
        let (_, cancellation_epoch) = self.hold_activation(true);
    ...
    async fn release(
        ...
        validate_installation_id(&installed.provider_env_installation_id)?;
        if installed.publication_generation == 0 {
            return Err(BackendError::Configuration(
                "configuration has no provider publication".to_string(),
            ));
        }
        let (_, cancellation_epoch) = self.hold_activation(true);
        let (response, epoch) = self
...
fn validate_installation_id(installation_id: &str) -> Result<(), BackendError> {
    let id = uuid::Uuid::parse_str(installation_id).map_err(|_| {
        BackendError::Configuration(
            "invalid provider environment installation identity".to_string(),
        )
    })?;
...
fn validate_prepared_receipt(
    ...
    validate_installation_id(&receipt.provider_env_installation_id)?;
    if receipt.expected_publication_generation != expected_publication_generation
        || receipt.publication_generation <= expected_publication_generation
        || receipt.provider_env_installation_id != installation_id
...
fn validate_installed_receipt(
    ...
    if receipt.identity != prepared.identity
        ...
        || receipt.publication_generation != prepared.publication_generation
        || receipt.provider_env_installation_id != prepared.provider_env_installation_id
    {
        return Err(BackendError::Confirm(
...
fn validate_released_receipt(
    ...
    if receipt.identity != installed.identity
        ...
        || receipt.publication_generation != installed.publication_generation
        || receipt.provider_env_installation_id != installed.provider_env_installation_id
    {
        return Err(BackendError::Confirm(
    ...
    loop {
        match stream.message().await {
            ...
            Err(error) => {
                ...
                // The request pump retains the split reader. Close this
                // write direction explicitly so response readers observe EOF.
                let _ = writer.shutdown().await;
                return;
            }
        }
...
mod tests {
    ...
    async fn configuration_client_with_service(
        service: TestGrpcBoundary,
    ) -> (Arc<BoundaryClient>, tokio::task::JoinHandle<()>) {
    ...
    async fn configuration_activation_rejects_wrong_start_and_exec_receipts() {
        ...
        let mut accepted = test_activation();
        accepted.provider_env_installation_id = credentials.snapshot().installation_id.clone();
        client.publish_activation(0, &accepted).unwrap();
        let context = sandbox();
    ...
    fn configuration_activation_receipts_bind_every_revision_coordinate() {
        ...
        variants.push(changed);
        let mut changed = original.clone();
        changed.provider_env_installation_id = uuid::Uuid::new_v4().to_string();
        variants.push(changed);
        let mut changed = original.clone();
    ...
    async fn configuration_discovery_waits_for_delayed_boundary_boot() {
        ...
        let server = tokio::spawn(async move {
            // The bootstrap exceeds the ordinary control deadline. Resume real
            // time before the TLS exchange so virtual timers cannot outrun I/O.
        ...
        let response = tokio::time::timeout(
            ...
            client.call_idempotent(Request::DescribeWorkload {
                supervisor_instance_id: client.supervisor_instance_id,
                resource_claims: std::collections::BTreeMap::new(),
            }),
        )
        .await
        .expect("bootstrap remains bounded")
        .expect("discovery receives the bootstrap grace period");
        assert!(matches!(response, Response::WorkloadDescribed { .. }));
        assert!(started.elapsed() >= Duration::from_secs(45));
        assert_eq!(requests.load(Ordering::Acquire), 1);
        assert!(
            client.attach_request.lock().unwrap().is_none(),
            "discovery must not attach the workload"
        );
        assert!(
            !*client.readiness().borrow(),
            "discovery must not release execution"
        );
        server.abort();
    }
...
provider_env_installation_id: None,
},
                                        processes: Vec::new(),
                                    },
                                }
...
provider_env_installation_id: installed.provider_env_installation_id.clone(),
    ...
    #[tokio::test]
    async fn grpc_response_error_closes_application_read_and_allows_reopen() {
        #[derive(Clone)]
        struct LostResponseBoundary {
            service: TestGrpcBoundary,
            lose_next_response: Arc<AtomicBool>,
        }

        #[tonic::async_trait]
        impl IsolationBoundary for LostResponseBoundary {
            type ExchangeStream = TestGrpcStream;
            type MediateStream = TestGrpcStream;
            ...
            async fn exchange(
                ...
                let response = self.service.exchange(request).await?;
                if !self.lose_next_response.swap(false, Ordering::AcqRel) {
                    return Ok(response);
                }
                // Lose a completed response after request processing, so the
                // client sees a stream failure rather than an RPC-open error.
                let stream = tokio_stream::StreamExt::map(response.into_inner(), |chunk| {
                    chunk.and(Err(tonic::Status::unavailable(
                        "injected completed response loss",
                    )))
                });
        ...
        let address = listener.local_addr().unwrap();
        let requests = Arc::new(std::sync::atomic::AtomicUsize::new(0));
        let lose_next_response = Arc::new(AtomicBool::new(false));
        let service = LostResponseBoundary {
            service: TestGrpcBoundary {
                wait_for_half_close: false,
                expected_token: "a".repeat(32),
                requests: requests.clone(),
            ...
            lose_next_response: lose_next_response.clone(),
        };
        let server = tokio::spawn(async move {
            let (stream, _) = listener.accept().await.unwrap();
        ...
        let request = encode_frame(&envelope).unwrap();

        for kind in [GrpcStreamKind::Exchange, GrpcStreamKind::Mediate] {
            lose_next_response.store(true, Ordering::Release);
            let mut failed = open_grpc_client_stream(channel.clone(), kind, &bearer)
                .await
                .unwrap();
            failed.write_all(&request).await.unwrap();
            // Keep the application write side open: the request pump still
            // owns its split reader when the response fails.
            let mut header = [0_u8; 4];
            let error =
                tokio::time::timeout(Duration::from_secs(1), failed.read_exact(&mut header))
                    .await
                    .expect("terminal gRPC error must wake the application reader")
                    .expect_err("a lost response must not produce a complete header");
            assert_eq!(error.kind(), std::io::ErrorKind::UnexpectedEof);
            drop(failed);

            let mut replacement = open_grpc_client_stream(channel.clone(), kind, &bearer)
                .await
                .unwrap();
            ...
            tokio::time::timeout(
                ...
                replacement.read_to_end(&mut response),
            )
            .await
            .expect("replacement stream must return the completed response")
            .unwrap();
            let response: ResponseEnvelope = decode_frame(&response).unwrap();
            assert_eq!(response.request_id, envelope.request_id);
            assert!(matches!(response.response, Response::Confirmed { .. }));
        }
    ...
    async fn persistent_dns_exchange_returns_supervisor_response() {
        ...
        let session = ClientMediationSession::start(Box::new(client_stream), 0);
        let server = tokio::spawn(async move {
